				  ...
				  struct platform_pwm_backlight_data *data)
{
	...
	int length;
    unsigned int *levels_value;
    unsigned int reversal = 0;
	u32 value;
	int ret;
    int i;
    ...
    /* determine the if reverse the brightness levels */
    of_property_read_u32(node, "brightness-reversal",&reversal);
	...
	if (data->max_brightness > 0) {
		size_t size = sizeof(*levels_value) * data->max_brightness;

		levels_value = devm_kzalloc(dev, size, GFP_KERNEL);
		if (!levels_value)
			return -ENOMEM;
        ...
        if(reversal > 0)
        {
            ...
            for(i=0; i<data->max_brightness; i++)
                {
                    data->levels[i] = levels_value[data->max_brightness-i-1];
                }
            devm_kfree(dev,levels_value);
        }
        else data->levels = levels_value;

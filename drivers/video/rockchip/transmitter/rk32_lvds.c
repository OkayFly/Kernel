...
static int rk32_lvds_pwr_on(void)
{
...
static int rk32_lvds_pwr_off(void)
{
     struct rk32_lvds *lvds = rk32_lvds;

     /* disable tx */
     writel_relaxed(0x12, lvds->regs + LVDS_CFG_REG_21);
     return 0;
}
...
static int rk32_lvds_en(void)
{
	...
	if (screen->type == SCREEN_RGB) {
		...
		writel_relaxed(0x00, lvds->regs + LVDS_CFG_REG_c); /*eanble pll*/
        while (delay_times--) {
            val = lvds_readl(lvds, LVDS_CH0_REG_f);
            if (val & 0x02) {     /* pll lock on */
                udelay(200);
                break;
            }
            udelay(100);
...
static struct rk_fb_trsm_ops trsm_lvds_ops = {
    ...
    .dsp_pwr_on = rk32_lvds_pwr_on,
    .dsp_pwr_off = rk32_lvds_pwr_off,
};

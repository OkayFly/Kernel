...
int es8323_earphone_con_gpio_active = 0;
...
char es8323_earphone_state = 0;
static int HP_IRQ=0;
static int hp_irq_flag = 0;
int LINE_IRQ = 0;
int line_in_state = 0;     // 0：line in out   1：line in in
...
static int es8323_read_gpio(int gpio)
{
	int i,level;
	for(i=0; i<3; i++)
	{
...
{
    es8323_earphone_state = es8323_read_gpio(es8323_hp_det_gpio);
    if(ctrl == 1) {
        if(es8323_earphone_state == 1)
        {
            if(es8323_earphone_con_gpio != INVALID_GPIO)                                                          
                    gpio_set_value(es8323_earphone_con_gpio, es8323_earphone_con_gpio_active);
            if(es8323_spk_con_gpio != INVALID_GPIO)                                                               
                    gpio_set_value(es8323_spk_con_gpio, !es8323_spk_con_gpio_active);
        }
        else 
        {
            if(es8323_earphone_con_gpio != INVALID_GPIO)                                                   
...
static void line_detect_do_switch(struct work_struct *work)
{
	...
	level = es8323_read_gpio(es8323_line_det_gpio);

	if(level == 0) //line in in
	{
		line_in_state = 1;
        printk("%s Enter line in mode\n",__func__);
        spk_detect_do_switch(1);

        /**** Add line in to earphone directly ****/
        snd_soc_write(es8323_codec, 0x26, 0x00);
        snd_soc_write(es8323_codec, 0x27, 0x40);
        snd_soc_write(es8323_codec, 0x2a, 0x40);
        snd_soc_write(es8323_codec, 0x0b, 0x02);   //line1 
        //power on left analog input N left ADC
        snd_soc_write(es8323_codec, 0x03, 0x00);
        snd_soc_write(es8323_codec, 0x04, 0x0c);  //enable LOUT2 ROUT2
        
        snd_soc_write(es8323_codec, 0x09, 0x00);
        snd_soc_write(es8323_codec, 0x0a, 0x00);

        /**** Add line in function ****/
        /*snd_soc_write(es8323_codec, 0x26, 0x00);
        snd_soc_write(es8323_codec, 0x0b, 0x02);   //line1
        ...
        snd_soc_write(es8323_codec, 0x09, 0x66);
        snd_soc_write(es8323_codec, 0x0a, 0x00);*/
	...
	else          //line in out
	{
        ...
        snd_soc_write(es8323_codec, 0x0b, 0x82);   //line1            
        //power on left analog input N left ADC                       
        snd_soc_write(es8323_codec, 0x03, 0x59);                      
        snd_soc_write(es8323_codec, 0x04, 0x3c);  //enable LOUT2 ROUT2
        
        snd_soc_write(es8323_codec, 0x09, 0x88);
        snd_soc_write(es8323_codec, 0x0a, 0xf0);
...
static void on_off_ext_amp(int i)
{
	...
	if(hp_irq_flag == 0) spk_detect_do_switch(i);
...
//use these if turn off DVDD when suspend
    ...
    spk_detect_do_switch(1);
    if(line_in_state == 1) schedule_delayed_work(&line_wakeup_work, msecs_to_jiffies(250));
    return 0;
}
...
#endif
    if(es8323_earphone_con_gpio != INVALID_GPIO) {
	    ret = gpio_request(es8323_earphone_con_gpio, "earphone_gpio");
	    if (ret != 0) {
		    printk("%s request earphone_gpio error", __func__);
		    return ret;
	    }
	    //gpio_set_value(SPK_CON, 1);
	    gpio_direction_output(es8323_earphone_con_gpio,!es8323_earphone_con_gpio_active);
    }

    if(es8323_spk_con_gpio != INVALID_GPIO) {
        ret = gpio_request(es8323_spk_con_gpio, "spk_gpio");
        if (ret != 0) {
            printk("%s request spk_gpio error", __func__);
            return ret;
        }
        //gpio_set_value(SPK_CON, 1);
        gpio_direction_output(es8323_spk_con_gpio,!es8323_spk_con_gpio_active);
    }
		...
		const struct i2c_device_id *id)
{
	...
	es8323_earphone_con_gpio = of_get_named_gpio_flags(i2c->dev.of_node, "earphone-con-gpio", 0, &flags);
	if (es8323_earphone_con_gpio < 0) {
		DBG("%s() Can not read property codec-earphone-en-gpio\n", __FUNCTION__);
		es8323_earphone_con_gpio = INVALID_GPIO;
	}
    es8323_earphone_con_gpio_active = !(flags & OF_GPIO_ACTIVE_LOW);

    es8323_spk_con_gpio = of_get_named_gpio_flags(i2c->dev.of_node, "spk-con-gpio", 0, &flags);
    if (es8323_spk_con_gpio < 0) {
        DBG("%s() Can not read property codec-en-gpio\n", __FUNCTION__);
        es8323_spk_con_gpio = INVALID_GPIO;
    }
    es8323_spk_con_gpio_active = !(flags & OF_GPIO_ACTIVE_LOW);
    ...
    of_property_read_u32(i2c->dev.of_node, "es8323-adc", &es8323_adc);
    printk("es8323_adc = %d\n",es8323_adc);
    if(es8323_adc)
    {
    ...
    printk("jp es8323_line_det_gpio:%d\n",es8323_line_det_gpio);
